...
export default function TimelineChart({ labels, datasets, title }) {
  ...
  dataProps.datasets = datasets.map((dataset, idx) => ({
    ...
    ...Object.fromEntries(Object.entries(DEFAULT_DATASET_OPTIONS)
      .map(entry => [entry[0], overrideIfUndefined(dataset[entry[0]], 
          typeof entry[1] === 'function' ? entry[1](idx) : entry[1]
        )]))
  }));
  ...
  return (
    <Line data={ dataProps } height={300} options={{ 
      plugins: plugins,
      responsive: true,
